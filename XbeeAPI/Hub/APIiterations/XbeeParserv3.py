#!/usr/bin/python
# XbeeAPI used in the Hub
# Allows for addressing each invidiual node on a network as well as Node discovery
# Can determine whether nodes are within range or out of range
# Is the coordinator of the network.
# Handles packet fragmentation and assembly
# Will be able to give much more data regarding network status 
 
import random, serial, time, threading, requests, os, unicodedata
 
from enum import Enum

# The frame types we expect
...
class MessagePacket:
    ...
    FRAME_TERMINATION_CHAR = '!' # Character that signifies final frame
    ...
    def _unloadPacket(self):
 	...
 	# Assemble frame to whole message
    def appendPayload(self, packet, nodename):
        ...
        if self.end == True:
        	print "Received full frame:",self.getPayloadAsString()
 
 	# Returns 64bit source address as series of bytes
    def getSource(self):
        return self.source

	# Returns 64bit source address as series of bytes in a string
    def getSourceAsString(self):
    	return self.source.decode("utf-8")
 
    def getPayload(self):
        return self.payload
 
 	# Returns entire assembled payload as ascii string
    def getPayloadAsString(self):
        return unicodedata.normalize('NFKD', self.payload.decode("utf-8")).encode('ascii', 'ignore')
 
 	# Will try to return payload if assembly is complete, else returns None
    def getContents(self):
        return self.getPayloadAsString() if self.endOfFragmentation() == True else None
 
 	# Verifies whether assembly is completed
    ...
    # Returns name of node on the network
...
class XbeeAPI:
    ...
    RESERVED = bytearray(b"\x7E\x7D\x11\x13") # Reserved hex values for escaping
    ...
    MAX_WAIT_HEARTBEAT = 10 # Maximum amount of bytes that can be transferred per frame
    TRANSMIT_OPTIONS = 0x00 # Default to 0
    ...
    heartbeatNodes = {} # Whether known nodes are in range or not
 ...
 # Serial buffer to handle data coming into the pi from serial
    ...
    def __init__(self, serialPort):
        ...
        # Add broadcast as addressable option to destination nodes
        self.destinationNodes['broadcast'] = '00 00 00 00 00 00 FF FF'
        # rxBuffer stores everything coming in from serial in the form of bytes
        # it wraps around also, so that packets that may be malformed from
        # cutting off reading can be collected
        self.rxBuffer = bytearray()
        self.triggered = False

        # Creates two threads, one that reguarly checks the network conditions
        # of nodes and the other that reads in any and all data it receives on serial
        self.readThread = threading.Thread(target=self.readSerial).start()
        self.heartBeatThread = threading.Thread(target=self._heartbeat).start()

    	# Reach out to any known nodes on the network
        self._getKnownNodes()
 
 	# Takes two strings, one of the nickname for a 64 bit address
 	# The other for the message to send it, i.e ('sensor1', 'hello world')
    def sendMessage(self, node, message):
		receiver = None
		if isinstance(node, str): # Will see if a string has been passed 
			address = self.getAddress(node) # Is the address on our known nodes list?
			if address == None:
				# No it's not, return here
				return 2 # Invalid address
			address = address.replace('0x0', '0x00').replace('0x','')	
			receiver = bytearray.fromhex(address)
		else: # Else a series of bytes has been passed
			receiver = node;
 	
 		# Begin the process of fragmenting the packet, return success/failure
    ...
    def getAddressName(self, node):
 	...
 	# Add a destination our known nodes list
    def _addDestination(self, node):
        for nodes in self.destinationNodes.values():
			...
			if nodes == newNode: # We already have this node stored
				return
        ...
        if self.receivedStatus == None or self._txStatus() == False: # Failed to contact the node
			return
		# Get any packets with HB#:
        packet = self._searchForPacketContaining("HB#:")

        # If no packet exists return
        if packet == None:
			return
		# Else save new node
        self._saveToFile(packet)
 
 	# Search all packets and return the first starting with a search string
    ...
    def _transmitMessage(self, destination, message):
        ...
        framesNeeded = int(round((len(message) / self.MAX_TRANSMIT_RF_DATA)+0.5)) # Calculate amount of frames required

        # Can only send a maximum of 255 frames (1 byte)
        if framesNeeded > 255:
        	return 1
        ...
        for i in range(0, framesNeeded):
            ...
            # Try 10 times to transmit frame
 	...
 	# Works out what part of the message being transmitted to disect and use in the frame
    def _getMessageChunk(self, index, message):
 	...
 	# produce frame, adds length, checksum, content, address and escapes the frame.
    ...
    def _produceFrame(self, node, frameID, frameContent, payload):
        ...
        frame.append(0x01) # Required frame ID
        ...
        frame.extend(payload) # Add the payload to the frame
    ...
    def readSerial(self):
        try:
            while True:
                ...
                remaining = self.serial.inWaiting() # Read in the rest of data in buffer
    ...
    def _checkSum(self, packet):
 	...
 	# Testing purposes, deprecated - ignore
    def testHex(self):
    ...
    def _getKnownNodes(self):
        ...
        while not result == 0: # Broadcasting to every node failed
        ...
        for rxPacket in self.receivedMessages: # Loop through all received packets within the last 3 seconds
            if rxPacket.endOfFragmentation() and rxPacket.getPayloadAsString().startswith("HB#"): # If the packet was a heartbeat response
                ...
                # This node responded and should be noted
                self.heartbeatNodes[rxPacket.getSourceAsString()] = True
               	self._ackAndRemoveMessage(rxPacket);
 		f.close()
 		print "Finished finding nodes."
 
    def _heartbeat(self):
   	while True:
		time.sleep(self.MAX_WAIT_HEARTBEAT) #Sleep for 60 seconds
		# Set all nodes to not have responded
		for key in self.heartbeatNodes.keys():
			self.heartbeatNodes[key] = False
		# Message each individual node with heartbeat request
		self.sendMessage("broadcast", "HB#")
		...
		# Shows all nodes that are 'alive' on the network
		print "Printing ranges..."
		for key in self.heartbeatNodes.keys():
			print key,
			print " - ",
			print self.heartbeatNodes[key]
 
    def _ackAndRemoveMessage(self, packet):
        self.receivedMessages.remove(packet)    
 
 	# Saves a node locally
    def _saveToFile(self, packet):
		f = open(self.DESTINATION_NODES_FILE, 'a') # Open and append file
